...
namespace EscapeFromTarkovCheat.Feauters
{
    class Aimbot : MonoBehaviour
    {
        ...
        private void Aim()
        {
            //Disabled Aimbot as it broke with recent updates and I don't know enough about Unity or EFT internally to fix it, nor do I use Aimbot enough to care to.
            //Feel free to fix it though! <3
                ...
                foreach (var player in Main.GamePlayers)
                {
                    ...
                    if (destination != Vector3.zero && CaulculateInFov(destination) <= Settings.AimbotFOV)
                    {
                        ...
                        if (distanceOfTarget > distance)
                        {
                            ...
                            player.Player.ProceduralWeaponAnimation.
